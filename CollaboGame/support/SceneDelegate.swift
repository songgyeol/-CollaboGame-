...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    private func navigationBarConfiguration(_ controller: UINavigationController) {
        ...
        controller.navigationBar.backgroundColor = CustomColor.mainTintColor
        
        if #available(iOS 13.0, *) {
            let navBarAppearance = UINavigationBarAppearance()
            navBarAppearance.configureWithOpaqueBackground()
            navBarAppearance.backgroundColor = myColor.backGroundColor
            navBarAppearance.largeTitleTextAttributes = [NSAttributedString.Key.foregroundColor: CustomColor.mainTintColor, NSAttributedString.Key.font: UIFont.Pretandard(type: .Bold, size: 40) as Any]
